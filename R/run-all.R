...
#' @param suffix "" if standard analysis "_all" if time analysis
...
#' assing a unique (worker,firm) spell ID for consecutive years
#' @export
...
#' @export
Do.lfe.residualize <- function(input_data, residualize_var, discrete_vars=NULL, continuous_vars = NULL, return_new_vars = F) {
...
#' this simulates data with movers, stayers, and correlated firm and worker effects
#' @export
...
Do.load.raw.data <- function(opts){
  ...
  if (substr(opts$country,1,3)=='US-'){
    ...
    for(yr in year_set){
      ...
      wage_data <- setDT(readRDS(file=sprintf("%sW2/w2_%s.rds",opts$paths$raw,yr))) # note: these are CPI-adjusted wages
      wage_data <- wage_data[wages > minearn][firm_ID %in% private_sector_firms]; gc()
      wage_data %<>% merge(yob_data,by='worker_ID')
      wage_data <- wage_data[, list(worker_ID,year=yr,firm_ID,birth_year=yob,wages=log(wages),state)]; gc()
      # infer if this should be a state-specific sample and impose this restriction if it is
      unique_states <- wage_data[,unique(state)]
      possible_state <- substr(opts$country,4,5)
      if(possible_state %in% unique_states){
        flog.info("restricting to state %s",possible_state)
        wage_data <- wage_data[state==possible_state]; gc()
      }
      wage_data[, state := NULL]
      # firm size
      if(minfirmsize > 0){
        wage_data[, firmsize := .N, firm_ID]
        wage_data <- wage_data[firmsize >= minfirmsize]; wage_data[, firmsize := NULL]; gc()
      }
      # finish
      wdata %<>% rbind(wage_data); wage_data <- NULL; private_sector_firms <- NULL; gc()
    }
...
#    }
  ...
  if ( substr(opts$country,1,2) == "SW"  ){
    ...
    #Restrictions - now you should use filter
    # wdata=wdata[(wdata$year>=2001)& (wdata$year<=2003) ] # only one year keep
    # wdata=wdata[(wdata$monthsworked==12) ] # full time work
    wdata<-wdata[!((wdata$wages=="NA") | (wdata$age=="NA") | (wdata$year=="NA") | (wdata$worker_ID=="NA") | (wdata$firm_ID=="NA")),]  # delete missing obs
    wdata$firm_ID<-as.numeric(factor(rank(wdata$firm_ID))) #convert due to large ID
  }
...
#   #SW---------------
#   if (opts$country=='SW0005'){
#     load("L:\\Tibo\\qtrdata\\smfe-data-all.dat")
#     wdata = rdata
#     rm(rdata)

#     # use yearly earings
#     wdata[,wages:=logyearnings]

#     # Restrictions
#     wdata=wdata[(wdata$year>=2000)& (wdata$year<=2005) ] # only one year keep
#     wdata<-wdata[!((wdata$wages=="NA") | (wdata$age=="NA") | (wdata$year=="NA") | (wdata$worker_ID=="NA") | (wdata$firm_ID=="NA")),]  # delete missing obs

#     # imposing floor
#     wage_ref = wdata[,mean(wages)]
#     wage_floor = wage_ref + log(0.325)
#     wdata=wdata[wages > wage_floor] # same restriction as IT lower floor
...
# #     if (False) {
# #
# #       wdata[monthsworked==12,mean(wages)]
# #       wdata[monthsworked==12,mean(logyearnings) -log(12)]
# #       wdata[,mean(wages)]
# #       wdata[,median(logyearnings) -log(12)]
# #       wage_ref = wdata[,mean(logyearnings) -log(12)]
# #
# #       rr = data.frame()
# #       for (share in (0:8)*0.1) {
# #         r = list(share=share)
# #         r$threshold =  wage_ref  + log(share)
# #         r$wage_fy_var = wdata[monthsworked==12][wages>r$threshold,var(wages)]
# #         r$wage_fy_count = wdata[monthsworked==12][wages>r$threshold,.N]
# #         r$wage_all_var = wdata[wages>r$threshold,var(wages)]
# #         r$wage_all_count = wdata[wages>r$threshold,.N]
# #         r$earnings_fy_var = wdata[monthsworked==12][logyearnings - log(12)>r$threshold,var(wages)]
# #         r$earnings_fy_count = wdata[monthsworked==12][logyearnings - log(12)>r$threshold,.N]
# #         r$earnings_all_var = wdata[logyearnings - log(12)>r$threshold,var(wages)]
# #         r$earnings_all_count = wdata[logyearnings - log(12)>r$threshold,.N]
# #         rr = rbind(rr,data.frame(r))
# #       }
# #     }
#   }

#   if (opts$country=='SW0005ft'){
#     load("L:\\Tibo\\qtrdata\\smfe-data-all.dat")
#     wdata = rdata
#     rm(rdata)
#     wdata[,wages:=logyearnings]
...
#     #Restrictions
#     wdata=wdata[(wdata$year>=2000)& (wdata$year<=2005) ] # only one year keep
#     wage_floor = wdata[,quantile(wages,0.278,na.rm=T)]
#     wdata=wdata[(wdata$monthsworked==12) ] # full time work
#     wdata=wdata[wdata$wages > wage_floor] # same restriction as IT lower floor
...
#   }
  ...
  if(opts$country %in% c('NO6brad-annualearnings')){
    wdata <- setDT(readRDS(file='/ssb/stamme01/swp/bjs/wk24/private/BHLMMS/raw/sample.rds'))
  }
  if(opts$country %in% c('NO6brad-annualearnings-educgender')){
    wdata <- setDT(readRDS(file='/ssb/stamme01/swp/bjs/wk24/private/BHLMMS/raw/sample.rds'))
    wdata[, educgender := .GRP, list(educ,male)]
  }
  if(opts$country %in% c('NO6brad-annualearnings-educindustry')){
    wdata <- setDT(readRDS(file='/ssb/stamme01/swp/bjs/wk24/private/BHLMMS/raw/sample.rds'))
    wdata[, educindustry := .GRP, list(educ,industry)]
  }
  if(opts$country %in% c('NO6brad-annualearnings-industry')){
    wdata <- setDT(readRDS(file='/ssb/stamme01/swp/bjs/wk24/private/BHLMMS/raw/sample.rds'))
    wdata[, industry := .GRP, list(industry)]
  }
  if(opts$country %in% c('NO6brad-annualearnings-educ')){
    wdata <- setDT(readRDS(file='/ssb/stamme01/swp/bjs/wk24/private/BHLMMS/raw/sample.rds'))
    wdata[, educ := .GRP, list(educ)]
  }
...
Do.misc.sample.construction <- function(wdata,opts){
  ...
  if(str_detect(opts$country,'noendpoints')){
    ...
    employed6 <- employed6[spells_per_worker <= 2]; gc()
...
Do.sample.init <- function(opts,data_filter_function = function(d) d){
  ...
  if (grepl('ITKline2', opts$country)==F) { #only ITKline 2 years no age residualization
      wdata <- Do.lfe.residualize(input_data=wdata, residualize_var="wages",discrete_vars="year",
                              continuous_vars = c("ones","agesq","agecu"), return_new_vars = T)
  }
  wdata[, `:=`(ones = NULL, agesq = NULL, agecu = NULL)]; gc()
  wdata[, wages := NULL]
  ...
  wdata <- NULL; gc() # help manage memory
  return(NULL)
}
...
Do.AKM.estimate.noIO <- function(ad){
  ...
  # reshape data
  long_data <- rbindlist(list( ad$jdata[,list(worker_ID = wid, firm_ID = f1, wages_pt = y1)],
...
Do.AKM.without.Spell.Collapse <- function(opts){
  ...
  # estimate AKM
  long_data <- wdata[,list(worker_ID,firm_ID,year,wages_pt=wages)]; gc()
  if('akm' %in% .packages()){
    flog.info("[Do.AKM] Package akm found. Estimating AKM on the jdata with akm.estimate.zig_zag.")
    firm_effects <- akm.estimate.zig_zag(long_data, outcome_var = "wages_pt", tol = 0, max_iterations = 100, report_frequency=10)$firms_with_fixedeffects
  } else {
...
Do.interacted.decomp <- function(suffix,opts){
  ...
  full_data <- rbindlist(list( ad$jdata[,list(worker_ID = wid, firm_ID = f1, cluster = j1, wages_pt = y1)],
...
Do.Sample.Clusters <- function(opts){
  ...
  for(suffix in CRE_suffixes){
    ...
    # shape data long
    full_data <- rbindlist(list( ad$jdata[,list(worker_ID = wid, firm_ID = f1, cluster = j1, y = y1)],
...
Do.EventStudy.6year <- function(opts){
  ...
  wdata <- wdata[,.(worker_ID,firm_ID,year,wages,wages_predict,ideal_mover)]; gc()
...
Do.Sample.Descriptives <- function(opts){
  ...
  # get characteristics of the clusters and sorting in the CRE results
  # if(any(!is.na(opts$suffix.set$CRE))){
  #   flog.info("Collecting cluster/sorting info from CRE estimates")
  #   Do.Sample.Clusters(opts)
  # }

  # get interacted quantiles and moving up/down study from interacted results
  # if(any(!is.na(opts$suffix.set$interacted))){
  #   flog.info("Collecting quantiles info from interacted estimates")
  #   Do.Quantiles.Interacted(opts)
  #   flog.info("Collecting sorting info from interacted estimates")
  #   Do.Interacted.Sorting(opts)
  # }
...
Do.run.all<-function(country,preserve.estimates=TRUE,preserve.initdata=TRUE,preserve.rawdata=TRUE,posterior_var=TRUE,data_only=FALSE, connectivity=FALSE){
  ...
  # Do.TraceHO.execute(opts,diff=TRUE)
  ...
  # Do.TraceHE.execute(opts,diff=TRUE)
  Do.TraceHE.execute(opts,diff=FALSE)

  # flog.info("3e. [Do.run.all] Estimate BLM with interacted, then collect decompositions for country %s",country)
  # Do.interacted.execute(opts)
