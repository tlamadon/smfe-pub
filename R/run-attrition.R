...
Do.attrition.draws.connected <- function(opts){
  ...
  for(draw in draws){
    ...
    if(opts$attrition.exercise$samefirms){
      ...
      ad$sdata <- ad$sdata[(f1 %in% remaining_firms)]; gc()

      # find connected set and only keep those firms
      ad <- Do.jdata_sdata.connected(ad)
      remaining_firms <- ad$jdata[,unique(c(f1,f2))]
      ad_draw$jdata <- ad_draw$jdata[f1 %in% remaining_firms & f2 %in% remaining_firms]; gc()
      ad_draw$sdata <- ad_draw$sdata[(f1 %in% remaining_firms)]; gc()
  ...
  run_leaveout <- function(ii){
    ...
    if (output_exists & opts$misc$preserve.estimates){
      ...
      if(opts$attrition.exercise$samefirms){
        ...
        ad_temp$sdata <- ad_temp$sdata[(f1 %in% remaining_firms)]; gc()

        # find connected set and only keep those firms
        ad_temp <- Do.jdata_sdata.leaveout(ad_temp)
        remaining_firms <- ad_temp$jdata[,unique(c(f1,f2))]
        ad$jdata <- ad$jdata[f1 %in% remaining_firms & f2 %in% remaining_firms]; gc()
        ad$sdata <- ad$sdata[(f1 %in% remaining_firms)]; gc()
    ...
    if(any_connected){
      ...
      if('CRE' %in% methods.connected){
        ...
        if (output_exists & opts$misc$preserve.estimates){
          ...
          #   cre.sub <- c(50,1000)
          # }
          # res <- Do.CRE_both.stats(m2.mini.estimate(jdata=ad$jdata,sdata=ad$sdata,method="linear.cre", cre.sub=cre.sub, include_covY1_terms=FALSE, posterior_var=opts$misc$posterior_var), suffix='_connected')
    ...
    if(any_leaveout){
      ...
      if('CRE' %in% methods.leaveout){
        ...
        if (output_exists & opts$misc$preserve.estimates){
          ...
          #   cre.sub <- c(50,1000)
          # }
          # res <- Do.CRE_both.stats(m2.mini.estimate(jdata=ad$jdata,sdata=ad$sdata,method="linear.cre", cre.sub=cre.sub, include_covY1_terms=FALSE, posterior_var=opts$misc$posterior_var), suffix='_leaveout')
...
#' @param country
#' @export
Do.run.attrition <- function(country,preserve.rawdata=TRUE,
                             preserve.estimates=TRUE,preserve.initdata=TRUE,data_only=TRUE,
                             draws=NA,moversperfirm=NA,
                             cluster_bothyears=FALSE, samefirms=FALSE){
